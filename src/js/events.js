...
export const initEvents = () => {
  ...
  const { refreshManualEdits, refreshInputOutput, refreshHorizontal } =
    appState.get();
  document
    .querySelectorAll('[data-action="toggle-refresh-opt"]')
    .forEach((el) => {
      if (el.dataset.opt === "refreshManualEdits")
        el.checked = refreshManualEdits;
      if (el.dataset.opt === "refreshInputOutput")
        el.checked = refreshInputOutput;
      if (el.dataset.opt === "refreshHorizontal")
        el.checked = refreshHorizontal;
    });
  ...
  document.getElementById("refresh-btn")?.addEventListener("click", () => {
    ...
    const newNodes = MathLogic.refreshLayout(
      nodes,
      edges,
      rowPreferences,
      rowCapacity,
  ...
  document.getElementById("export-btn")?.addEventListener("click", async () => {
    ...
    const cleanEdges = edges.map(
      ({ startX, startY, endX, endY, color, edgeIdx, ...rest }) => rest,
    );
...
const handleFileRead = async (file) => {
  try {
    ...
    if (!data.NODES || !Array.isArray(data.NODES)) {
      throw new Error(
        'Invalid JSON structure: Missing or invalid "NODES" array.',
      );
    }
    if (!data.EDGES || !Array.isArray(data.EDGES)) {
      throw new Error(
        'Invalid JSON structure: Missing or invalid "EDGES" array.',
      );
    }
    if (!data.NODE_TYPES || typeof data.NODE_TYPES !== "object") {
      throw new Error(
        'Invalid JSON structure: Missing or invalid "NODE_TYPES" object.',
      );
    }
...
const handleClicks = (e) => {
  ...
  if (!target || action === "drag-node") {
    const nodeEl = e.target.closest(".d3-node");
    if (nodeEl) {
    ...
    if (nodeEl) {
      ...
      if (action !== "select-node") {
        ...
        appState.set({
          selectedNode: selectedNode === clickedNodeId ? null : clickedNodeId,
        });
        return;
      }
    }
  ...
  switch (action) {
    ...
    case "move-unlock-down":
      ...
      const selectEl = document.getElementById(
        isPrereq ? `prereq-select-${nodeId}` : `unlock-select-${nodeId}`,
      );
      ...
      let newOptionsData = options.map((o) => ({
        value: o.value,
        text: o.text,
      }));
      ...
      const newlySelectedOptions = selectedIndices.map(
        (idx) => newOptionsData[idx].value,
      );
      if (isPrereq) {
        appState.set({
          edges: newEdges,
          selectedPrereqs: newlySelectedOptions,
        });
      } else {
        appState.set({
          edges: newEdges,
          selectedUnlocks: newlySelectedOptions,
        });
      }
      UI.updateEdges();
...
const handleChanges = (e) => {
  const target = e.target.closest(
    '[data-action], [id^="prereq-select-"], [id^="unlock-select-"]',
  );
  if (!target) return;
  ...
  if (target.dataset.action === "change-row-pref") {
    ...
    target.dataset.action === "change-time-year" ||
    target.dataset.action === "change-time-sem"
  ) {
    ...
    // Get current values
    let currentYear =
      node.manualRow !== undefined ? Math.floor(node.manualRow / 3) + 1 : null;
    let currentSem =
      node.manualRow !== undefined ? (node.manualRow % 3) + 1 : null;
...
const handleHoverOut = (e) => {
  ...
  if (nodeEl && !e.relatedTarget?.closest(".d3-node")) {
